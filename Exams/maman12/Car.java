...
public class Car {
	...
	// default values
	private static final int DEFAULT_ID_VAL = 9999999;
	private static final char DEFAULT_TYPE_VAL = 'A';

	// best and worst car's type characters
	private static final char BEST_TYPE_VAL = 'D';
	private static final char WORST_TYPE_VAL = 'A';

	// CONSTRUCTORS
	/**
	 * creates a new Car object id should be a 7 digits number, otherwise set it to
	 * 9999999 type should be 'A','B','C' or 'D', otherwise set it to 'A'
	 * 
	...
	public Car(int id, char type, String brand, boolean isManual) {
		_id = (isValidCarId(id)) ? id : DEFAULT_ID_VAL; // If 'id' is correct, set value received as parameter,
														// otherwise, default value.
		_type = (isValidCarType(type)) ? type : DEFAULT_TYPE_VAL; // If 'type' is correct, set value received as
																	// parameter, otherwise, default value.
		_brand = brand;
		_isManual = isManual;
	}
	...
	public void setId(int id) {
		if (isValidCarId(id)) // Checks whether the 'id' that received as a parameter is valid, if it is,
								// change the id.
			_id = id;
	}
	...
	public void setType(char type) {
		if (isValidCarType(type)) // Checks whether the 'type' that received as a parameter is valid, if it is,
									// change the type.
			_type = type;
	}
	 ...
	 * 
	 * @return String that represents this car in the following format: id:1234567
	 *         type:B brand:Toyota gear:manual (or auto)
	 */
	public String toString() {
		return "id:" + _id + " type:" + _type + " brand:" + _brand + " gear:" + (_isManual ? "manual" : "auto");
		// string representation of an using ternary operator to determine if the gear
		// is auto or manual
	}

	/**
	 * Check if two cars are the same Cars are considered the same if they have the
	 * same type, brand and gear
	 * 
	...
	/**
	 * Check if this car is better than the other car A car is considered better
	 * than another car if its type is higher. If both cars have the same type, an
	 * automated car is better than a manual car.
	 * 
	 * @param other the car to compare this car to
	...
	public boolean better(Car other) {
		if (_type > other._type) // Check whether 'this' object type is higher than 'other' type.
			...
			return false;
		return !_isManual && other._isManual; // Checks whether the 'this' object gear is auto and (logical AND operator
												// '&&') the 'other' object gear is manual, return true if both operands
												// are true, otherwise, return false.
	}

	/**
	 * Check if this car is worse than the other car The logic of the method is that
	 * if the 'other' car is better than 'this' car, then 'this' car must be worse
	 * than 'other' car
	 * 
	 * @param other the car to compare this car to
	...
	public boolean worse(Car other) {
		return other.better(this); // A call to the 'better' method when the method is invoked on 'other', where
	...
	private boolean isValidCarId(int id) {
		return String.valueOf(id).length() == String.valueOf(DEFAULT_ID_VAL).length(); // Checks whether the length of
																						// the ID as a string is valid
	...
	private boolean isValidCarType(char type) {
		return type >= WORST_TYPE_VAL && type <= BEST_TYPE_VAL; // Checks whether the type is correct by complying with
																// the range between A and D according to the ASCII
																// code.
	}
}